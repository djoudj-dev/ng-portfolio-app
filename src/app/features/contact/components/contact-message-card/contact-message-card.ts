...
@Component({
  ...
  imports: [ButtonComponent, ReactiveFormsModule, NgClass, ContactStatusMessage],
    ...
    <div class="min-h-[500px]">
      @if (formSubmissionStatus() === 'idle') {
        ...
        <form [formGroup]="contactForm" (ngSubmit)="onSubmit()" class="space-y-4">
          ...
          <div>
            <label for="name" class="text-text mb-1 block text-sm font-medium">Nom</label>
            <input
              type="text"
              id="name"
              ...
              [ngClass]="{
                'border-red-500': (formSubmitted() || nameTouched()) && nameControl?.invalid,
              }"
              (blur)="nameTouched.set(true)"
            />
            @if ((formSubmitted() || nameTouched()) && nameControl?.errors?.['required']) {
              <p class="mt-1 text-xs text-red-500">Le nom est requis</p>
            }
            @if ((formSubmitted() || nameTouched()) && nameControl?.errors?.['minlength']) {
              <p class="mt-1 text-xs text-red-500">Le nom doit contenir au moins 3 caractères</p>
            }
          </div>

          <div>
            <label for="email" class="text-text mb-1 block text-sm font-medium">Email</label>
            <input
              type="email"
              id="email"
              ...
              [ngClass]="{
                'border-red-500': (formSubmitted() || emailTouched()) && emailControl?.invalid,
              }"
              (blur)="emailTouched.set(true)"
            />
            @if ((formSubmitted() || emailTouched()) && emailControl?.errors?.['required']) {
              <p class="mt-1 text-xs text-red-500">L'email est requis</p>
            }
            @if ((formSubmitted() || emailTouched()) && emailControl?.errors?.['email']) {
              <p class="mt-1 text-xs text-red-500">Veuillez entrer une adresse email valide</p>
            }
          </div>

          <div>
            <label for="subject" class="text-text mb-1 block text-sm font-medium">Sujet</label>
            <input
              type="text"
              id="subject"
              ...
              class="bg-background text-text border-primary focus:ring-accent w-full rounded-lg border px-4 py-2 focus:ring-2 focus:outline-none"
              [ngClass]="{
                'border-red-500': (formSubmitted() || subjectTouched()) && subjectControl?.invalid,
              }"
              (blur)="subjectTouched.set(true)"
            />
            @if ((formSubmitted() || subjectTouched()) && subjectControl?.errors?.['required']) {
              <p class="mt-1 text-xs text-red-500">Le sujet est requis</p>
            }
          </div>

          <div>
            <label for="message" class="text-text mb-1 block text-sm font-medium">Message</label>
            <textarea
              id="message"
              ...
              class="bg-background text-text border-primary focus:ring-accent w-full rounded-lg border px-4 py-2 focus:ring-2 focus:outline-none"
              [ngClass]="{
                'border-red-500': (formSubmitted() || messageTouched()) && messageControl?.invalid,
              }"
              (blur)="messageTouched.set(true)"
            ></textarea>
            @if ((formSubmitted() || messageTouched()) && messageControl?.errors?.['required']) {
              <p class="mt-1 text-xs text-red-500">Le message est requis</p>
            }
            @if ((formSubmitted() || messageTouched()) && messageControl?.errors?.['minlength']) {
        ...
        <app-contact-status-message [status]="getStatusForComponent()" (tryAgain)="resetForm()" />
      }
    </div>
  `,
})
