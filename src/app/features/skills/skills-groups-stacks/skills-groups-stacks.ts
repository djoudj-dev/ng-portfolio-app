...
import { type SkillCategoryData } from '../interface/skill-data';
            ...
            @for (skill of category.skills; track skill.id; let skillIdx = $index) {
              <li [style.animation-delay]="skillIdx * 50 + 'ms'">
                @if (skill.url) {
                  ...
                  >
                    <div
                      class="flex flex-shrink-0 items-center justify-center transition-transform group-hover/skill:scale-110 duration-300"
                    >
                      <app-svg-icon
                        [name]="skill.icon"
                        [width]="'24'"
                    ...
                    <h3
                      class="text-xs sm:text-sm font-semibold text-primary/70 truncate min-w-0 group-hover/skill:text-accent transition-colors uppercase tracking-150 leading-none"
                    >
                      {{ skill.title }}
                    </h3>
                  </a>
                  ...
                  >
                    ...
                    <h3
                      class="text-xs sm:text-sm font-semibold text-primary/70 truncate min-w-0 uppercase tracking-150 leading-none"
                    >
                      {{ skill.title }}
                    </h3>
                  </div>
...
export class SkillsGroupsStacks {
  readonly skillCategories: readonly SkillCategoryData[] = SKILL_CATEGORIES;
}
